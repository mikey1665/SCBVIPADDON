package mike1665.classpacks.vip;

import java.util.ArrayList;

import org.bukkit.ChatColor;
import org.bukkit.Color;
import org.bukkit.Material;
import org.bukkit.block.Block;
import org.bukkit.enchantments.Enchantment;
import org.bukkit.entity.Arrow;
import org.bukkit.entity.Entity;
import org.bukkit.entity.EntityType;
import org.bukkit.entity.Player;
import org.bukkit.event.block.Action;
import org.bukkit.inventory.ItemStack;
import org.bukkit.inventory.meta.ItemMeta;
import org.bukkit.inventory.meta.LeatherArmorMeta;

import Pauldg7.plugins.SCB.interfaces.ClassInterface;

public class Horse implements ClassInterface{

	@Override
	public void ArrowHit(Player arg0, Entity arg1) {
		// TODO Auto-generated method stub
		
	}

	@Override
	public void Attack(Player arg0, Player arg1) {
		// TODO Auto-generated method stub
		
	}

	@Override
	public void Death(Player arg0) {
		// TODO Auto-generated method stub
		
	}

	@Override
	public String DisplayName() {
		return ChatColor.DARK_RED + "[Horse] " + ChatColor.RESET; 
	}

	@Override
	public ItemStack Icon() {
		ItemStack icon = new ItemStack(Material.SADDLE);
	    ItemMeta im = icon.getItemMeta();
	    im.setDisplayName(ChatColor.DARK_AQUA + "Horse");
	    ArrayList<String> lore = new ArrayList<String>();
	    lore.add(ChatColor.YELLOW + "Summons a ridable horse");
	    lore.add(ChatColor.YELLOW + "Has a shootable apple!");
	    im.setLore(lore);
	    icon.setItemMeta(im);
	    return icon;
	}

	@Override
	public boolean Jump(Player arg0) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public boolean Movement(Player arg0) {
		// TODO Auto-generated method stub
		return false;
	}

	@SuppressWarnings("deprecation")
	@Override
	public void RightClick(Player p, Action arg1, Block arg2) {
		if (a(p)) {
			org.bukkit.entity.Horse h = (org.bukkit.entity.Horse) p.getWorld().spawnEntity(p.getLocation(), EntityType.HORSE);
			h.setOwner(p);
			h.setCarryingChest(true);
			h.setTamed(true);
			h.setColor(org.bukkit.entity.Horse.Color.CREAMY);
			h.setJumpStrength(2);
			h.getInventory().setSaddle(new ItemStack(Material.SADDLE));
			h.getInventory().setArmor(new ItemStack(Material.IRON_BARDING));	
			h.setAdult();
			h.setCustomName(ChatColor.GREEN + p.getName().toString() + "'s Horse");
			h.setPassenger(p);
		}
		if (b(p)) {
		     if (p.getItemInHand().getAmount() > 1) {
		    	 p.getItemInHand().setAmount(p.getItemInHand().getAmount() - 1);
		     }else {
		    	 p.getInventory().remove(p.getItemInHand());
		    	 p.setItemInHand(null);
		     	 p.updateInventory(); 
		     }
		    	 p.launchProjectile(Arrow.class);	    
		}
		
	}

	@Override
	public boolean ShootArrow(Player arg0) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public void Spawn(Player player) {

		ItemStack h = new ItemStack(Material.LEATHER_HELMET);
	    ItemStack c = new ItemStack(Material.LEATHER_CHESTPLATE);
	    ItemStack l = new ItemStack(Material.LEATHER_LEGGINGS);
	    ItemStack b = new ItemStack(Material.LEATHER_BOOTS);
	    
	    LeatherArmorMeta cam = (LeatherArmorMeta)c.getItemMeta();
	    LeatherArmorMeta lam = (LeatherArmorMeta)l.getItemMeta();
	    LeatherArmorMeta bam = (LeatherArmorMeta)b.getItemMeta();
	    LeatherArmorMeta helm = (LeatherArmorMeta)h.getItemMeta();
	    cam.setColor(Color.OLIVE);
	    lam.setColor(Color.OLIVE);
	    bam.setColor(Color.OLIVE);
	    helm.setColor(Color.OLIVE);
	    
	    c.setItemMeta(cam);
	    l.setItemMeta(lam);
	    b.setItemMeta(bam);
	    h.setItemMeta(helm);
	    b.addUnsafeEnchantment(Enchantment.PROTECTION_FALL, 10);
	    b.addUnsafeEnchantment(Enchantment.PROTECTION_ENVIRONMENTAL, 4);
	    
	    player.getPlayer().getInventory().setHelmet(h);
	    player.getPlayer().getInventory().setChestplate(c);
	    player.getPlayer().getInventory().setLeggings(l);
	    player.getPlayer().getInventory().setBoots(b);
	    
	    ItemStack i1 = new ItemStack(Material.WOOD_SWORD, 1);
	    ItemStack i2 = new ItemStack(Material.MONSTER_EGG, 1, (short)EntityType.HORSE.ordinal());
	    ItemStack i3 = new ItemStack(Material.ARROW, 20);
	    
	    i1.addUnsafeEnchantment(Enchantment.KNOCKBACK, 3);
	    
	    ItemMeta im1 = i2.getItemMeta();
        im1.setDisplayName(ChatColor.RED + "Pet Horse");
        ArrayList<String> im3l = new ArrayList<String>();
        im3l.add(ChatColor.YELLOW + "Spawns a horse");
        
        im1.setLore(im3l);
        i2.setItemMeta(im1);
	    
	    i1.addUnsafeEnchantment(Enchantment.DAMAGE_ALL, 3);
	    
	    player.getPlayer().getInventory().addItem(new ItemStack[] { i1,i2,i3 });
	}

	@Override
	public void ThrowPotion(Player arg0, Player arg1) {
		// TODO Auto-generated method stub
		
	}

	@Override
	public String id() {
		return "horse";
	}
	public boolean a(Player player) {
	    return (player.getItemInHand() != null && player.getItemInHand().getType() == Material.MONSTER_EGG);
	  }
	public boolean b(Player player) {
	    return (player.getItemInHand() != null && player.getItemInHand().getType() == Material.ARROW);
	  }
}
